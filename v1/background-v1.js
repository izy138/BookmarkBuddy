...
async function getAllBookmarks() {
  ...
  function traverse(nodes, rootFolder = "", folderPath = "") {
    for (const node of nodes) {
      ...
      if (node.children) {
        traverse(node.children, rootFolder, node.title || folderPath);
      }
    }
  }

  // Traverse each root-level folder separately to tag rootFolder
  for (const rootNode of rootChildren) {
    ...
    if (rootNode.children) {
      ...
      if (shoppingIdx !== -1) {
        ...
        traverse(shoppingNode.children || [], "Shopping List", "Shopping List");
        ...
        traverse(otherChildren, rootName, rootName);
        continue;
      }
    }
    ...
    if (rootNode.url) {
      ...
      traverse(rootNode.children, rootName, rootName);
    }
  }
